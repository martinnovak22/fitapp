...
- 🔨 **Phase 2 — Review UI** — in progress: a per-group review screen shown in the guest→account sign-in flow after the transition commits (see revised Q3). Needs Android verification.
...
3. ~~**When does merge run**~~ — **RESOLVED (revised — supersedes the earlier "standalone action" answer):** a **review step in the guest→account sign-in flow**, run *after* the atomic Principal Transition commits, only when detection finds Duplicate Groups. It still *cannot* live inside the transition (ADR-0002 atomicity vs the human-in-the-loop review list, Q2), so it's a post-commit follow-on step, per-group confirm, **skippable**. No standalone/maintenance action — so dupes from non-sign-in sources (LWW double-seed, manual typo) are **not** cleaned up (accepted; never data loss per ADR-0002). See [ADR-0005](../adr/0005-exercise-deduplication.md).
