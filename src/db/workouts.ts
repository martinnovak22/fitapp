...
export const WorkoutRepository = {
    ...
    async getActiveWorkout(): Promise<Workout | null> {
        const db = await getDb()
        const result = await db.getFirstAsync<Workout>('SELECT * FROM workouts WHERE status = ? LIMIT 1', 'in_progress')
        return result ?? null
    },

    async getAllWorkouts(): Promise<Workout[]> {
        const db = await getDb()
        return await db.getAllAsync<Workout>('SELECT * FROM workouts ORDER BY date DESC, start_time DESC')
    },

    async getWorkoutsForDate(date: string): Promise<Workout[]> {
        const db = await getDb()
        return await db.getAllAsync<Workout>('SELECT * FROM workouts WHERE date = ?', date)
    },
